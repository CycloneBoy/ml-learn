#!/user/bin/env python
# -*- coding: utf-8 -*-
# @File  : fp_growth_first.py
# @Author: sl
# @Date  : 2020/10/25 - 上午10:40
...
class FPNode(object):
    ...
    def add(self, child):
        ...
        if not isinstance(child, FPNode):
            raise TypeError("Can only add other FPNode as children")
...
# 查找频繁项集
def find_frequent_itemset(transactions, min_support,min_confidence, include_support=False):
    ...
    minimum_support = int(len(transactions) * min_support)
    print("支持度大小: {} -> 总的数据量：{} ->设置的支持度大小：{}".format(  minimum_support,len(transactions),min_support))
