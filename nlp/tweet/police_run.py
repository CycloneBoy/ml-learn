...
def predict_submission(test_data,model):
    ...
    submission_df_1.to_csv("{}/{}".format(data_dir, 'submission_2.csv'), index=False)



def predict_police(model):
...
def train_model(data_clean):
    ...
    report_model(X_test, model, y_test)
    return model


def report_model(X_test, model, y_test):
...
if __name__ == '__main__':
    ...
    data_clean = clean_train_data(train_data)

    model = train_model(data_clean)

    predict_submission(test_data,model)


    predict_police(model)
    pass
