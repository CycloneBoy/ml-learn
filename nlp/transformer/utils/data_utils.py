...
from nlp.transformer.utils.dataset import ParallelDataset
...
def convert_idx2text(examples, id2word):
    words = []
    for i in examples:
        if i == EOS:
            break
...
def load_train_data(data_path, batch_size, max_src_len, max_trg_len, use_cuda=False):
    ...
    def filter_pred(examples):
        if len(examples.src) <= max_src_len and len(examples.trg) <= max_trg_len:
            return True
        return False
    ...
    train_data = ParallelDataset(train_src, train_tgt, filds=fields, filter_pred=filter_pred, )
    train_iter = Iterator(dataset=train_data, batch_size=batch_size, train=True,  # Variable(volatile=False)
                          sort_key=lambda x: data.interleave_keys(len(x.str), len(x.trg)),
                          repeat=False, shuffle=True, device=device)

    dev_data = ParallelDataset(dev_src, dev_tgt, filds=fields)
    dev_iter = Iterator(dataset=dev_data, batch_size=batch_size, train=False,
                        repeat=False, shuffle=True, device=device, sort=False)
...
def load_test_data(data_path, vocab_path, batch_size, use_cuda=False):
    ...
    test_src = convert_text2id(read_corpus(data_path, None, lower_case), src_word2idx)
    test_data = ParallelDataset(test_src, None, filds=fields)
