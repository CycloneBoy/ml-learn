...
class Beam(object):
    ...
    def __init__(self, size, use_cuda=True):
        self.size = size
        self.done = False
    ...
    def advance(self, word_lk):
        ...
        best_scroes, best_scroes_id = flat_beam_lk.topk(self.size, 0, True, True)  # 1st sort
        best_scroes, best_scroes_id = flat_beam_lk.topk(self.size, 0, True, True)  # 2nd sort TODO

        self.all_scores.append(self.scores)
        self.scores = best_scroes

        # bestScoresId is flattened beam_size * tgt_vocab_size array, so calculate
        # which word and beam each score came from
        prev_k = best_scroes_id / num_words
        self.prev_ks.append(prev_k)
        self.next_ys.append(best_scroes_id - prev_k * num_words)
