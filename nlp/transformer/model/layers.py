...
from nlp.transformer.model.sublayers import MultiHeadAttention, PoswiseFeedForwardNet, MultiBranchAttention


class EncoderLayer(nn.Module):
    def __init__(self, d_k, d_v, d_model, d_ff, n_heads, dropout=0.1):
        super(EncoderLayer, self).__init__()
        self.enc_selt_attn = MultiHeadAttention(d_k, d_v, d_model, n_heads, dropout)
        self.pos_ffn = PoswiseFeedForwardNet(d_model, d_ff, dropout)

    def forward(self, enc_inputs, self_attn_mask):
        enc_outputs, attn = self.enc_selt_attn(enc_inputs, enc_inputs,
...
class WeightedEncoderLayer(nn.Module):
    def __init__(self, d_k, d_v, d_model, d_ff, n_branches, dropout=0.1):
        ...
        self.enc_selt_attn = MultiHeadAttention(d_k, d_v, d_model, d_ff, n_branches, dropout)

    def forward(self, enc_inputs, self_attn_mask):
        return self.enc_selt_attn(enc_inputs, enc_inputs, enc_inputs, attn_mask=self_attn_mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, self_attn_mask, enc_attn_mask):
        dec_outputs, dec_self_attn = self.enc_self_attn(dec_inputs, dec_inputs, dec_inputs, attn_mask=self_attn_mask)
...
class WeightedDecoderLayer(nn.Module):
    def __init__(self, d_k, d_v, d_model, d_ff, n_branches, dropout=0.1):
        ...
        self.dec_enc_attn = MultiBranchAttention(d_k, d_v, d_model, n_branches, dropout)
